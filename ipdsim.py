...
#   python ipdsim.py -s allC allD TFT TFTd TFTdcc TF2T GRIM
...
numberOf_TF2T   =  16   # Tit-for-2-Tats (cooperate, defect when opponent defects twice in a row)
...
if '-h' in argv or '-help' in argv or '--help' in argv:
    ...
    print("    TF2T --- 'Tit-for-2-Tats' C first, D when opponent plays D twice in a row;")
    ...
    print("    python ipdsim.py -s allC allD TFT TFTd TFTdcc TF2T GRIM")
...
if '-s' in argv:
    try:
        ...
        numberOf_TF2T  = int(strategyDistribution[6])
    except (ValueError, IndexError):
        print("To alter the initial distribution of strategies, use the syntax:")
        print("    python ipdsim.py -s allC allD TFT TFTd TFTdcc TF2T GRIM")
        print("    NOTE: allC, allD, etc. are integers. Six values must be given")
...
N = (numberOf_allC + numberOf_TFT + numberOf_TFTd + numberOf_TFTdcc +
     numberOf_GRIM + numberOf_allD + numberOf_TF2T)
if cull > N/2:
...
def updateDistribution():
    ...
    for n in G:
        ...
        if G.node[n]['abbr'] == 'TF2T'  : count_TF2T   += 1
...
def printDistributionHistory():
    header='Round: ---  allC   allD    TFT   TFTd TFTdcc TF2T  GRIM --- Average Score'
    print(header)
    for i in range(r):
        print(
            '%5d:    %6d %6d %6d %6d %6d %6d %6d    %13d'
            %
            (i,
            distribution[i][0][1],
            distribution[i][1][1],
    ...
    for i in range(iterations):
        ...
        if i == 2:
            ...
            if history_A[-1] == 'C':
                ...
                action_B = G.node[node_B]['react_D']
            if 'react_DD' in G.node[node_A]:
    	        if history_B[-2] == 'D' and history_B[-1] == 'D':
    	            action_A = G.node[node_A]['react_DD']
    	        else:
    	            action_A = G.node[node_A]['react_C']
            if 'react_DD' in G.node[node_B]:
    	        if history_A[-2] == 'D'and history_A[-1] == 'D':
	                action_B = G.node[node_B]['react_DD']
                else:
                    action_B = G.node[node_B]['react_D']
            if 'reactEverD' in G.node[node_A] and 'D' in history_B:
                action_A = G.node[node_A]['reactEverD']
...
def cullingAndSeeding():
    ...
    for i,(n,score,type) in enumerate(cullList):
        ...
        if seedList[i][2] == 'TF2T': agent.update(TF2T)
